...
const char * const BITCOIN_CONF_FILENAME = "anokas.conf";
const char * const BITCOIN_PID_FILENAME = "anokasd.pid";
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\AnokasCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\AnokasCore
    // Mac: ~/Library/Application Support/AnokasCore
    // Unix: ~/.anokascore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "AnokasCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/AnokasCore";
#else
    // Unix
    return pathRet / ".anokascore";
#endif
#endif
}
...
void ArgsManager::ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty anokas.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override anokas.conf
