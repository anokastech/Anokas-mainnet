...
bool GenerateDistributionList(const CRewardSnapshot& p_rewardSnapshot, std::vector<OwnerAndAmount>& vecDistributionList)
{
    ...
    CAmount srcUnitDivisor = COIN;  //  Default to divisor for ANOK
    ...
    if (p_rewardSnapshot.strDistributionAsset != "ANOK") {
        ...
        LogPrint(BCLog::REWARDS, "%s: Distribution is ANOK with divisor %d\n", __func__, srcUnitDivisor);
    }
        ...
        std::string& change_address, uint256& retTxid)
{
    ...
    //  Handle payouts using ANOK differently from those using an asset
    if (p_rewardSnapshot.strDistributionAsset == "ANOK") {
        // Check amount
        ...
        for (int i = start; i < (int)p_pendingPayments.size() && i < stop; i++) {
            ...
            // Parse Anokas address (already validated during ownership snapshot creation)
