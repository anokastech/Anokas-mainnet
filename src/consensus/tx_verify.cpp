...
bool CheckTransaction(const CTransaction& tx, CValidationState &state, int nHeight, CAmount blockReward, bool fMempoolCheck, bool fBlockCheck)
{
    ...
    for (const auto& txout : tx.vout)
    {
        ...
        /** ANOK START */
        // Find and handle all new OP_ANOK_ASSET null data transactions
    ...
    if (tx.IsNewAsset()) {
        ...
        // above transaction types.  Also fail if it contains OP_ANOK_ASSET opcode but wasn't a valid script.
        for (auto out : tx.vout) {
            int nType;
            bool _isOwner;
            if (out.scriptPubKey.IsAssetScript(nType, _isOwner)) {
                ...
                if (out.scriptPubKey.Find(OP_ANOK_ASSET)) {
                    if (out.scriptPubKey[0] != OP_ANOK_ASSET) {
                        return state.DoS(100, false, REJECT_INVALID,
                                         "bad-txns-op-ANOK-asset-not-in-right-script-location");
                    }
                }
            }
        }
