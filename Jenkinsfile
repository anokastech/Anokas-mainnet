...
for(int i = 0; i < targets.size(); i++) {
  ...
  tasks["${target}"] = {
    node {
      ...
      withEnv(env) {
        def builderImageName="anokas-builder-${target}"
        ...
        builderImage.inside("-t") {
          ...
          def hasCache = false
          try {
            copyArtifacts(projectName: "anokas-anokas/${BRANCH_NAME}", optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz")
          } catch (Exception e) {
          }
          if (fileExists("ci-cache-${target}.tar.gz")) {
            hasCache = true
            echo "Using cache from anokas-anokas/${BRANCH_NAME}"
          } else {
            try {
              copyArtifacts(projectName: 'anokas-anokas/develop', optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz");
            } catch (Exception e) {
            }
            if (fileExists("ci-cache-${target}.tar.gz")) {
              hasCache = true
              echo "Using cache from anokas-anokas/develop"
            }
          }
          ...
          if (hasCache) {
            ...
            sh "mkdir -p /anokas-src/ci-cache-${target}"
          }

          stage("${target}/depends") {
            sh 'cd /anokas-src && ./ci/build_depends.sh'
          }
          stage("${target}/build") {
            sh 'cd /anokas-src && ./ci/build_src.sh'
          }
          stage("${target}/test") {
            sh 'cd /anokas-src && ./ci/test_unittests.sh'
          }
          stage("${target}/test") {
            sh 'cd /anokas-src && ./ci/test_integrationtests.sh'
          }

          // archive cache and copy it into the jenkins workspace
          sh "cd /anokas-src && tar czfv ci-cache-${target}.tar.gz ci-cache-${target} && cp ci-cache-${target}.tar.gz ${pwd}/"
        }
