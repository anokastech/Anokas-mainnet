...
void CDKGSession::SendContributions(CDKGPendingMessages& pendingMessages)
{
    ...
    for (size_t i = 0; i < members.size(); i++) {
        ...
        if (!qc.contributions->Encrypt(i, m->dmn->pdmnState->pubKeyOperator.Get(), skContrib, MIN_SMARTNODE_PROTO_VERSION)) {
...
void CDKGSession::ReceiveMessage(const uint256& hash, const CDKGContribution& qc, bool& retBan)
{
    ...
    if (!qc.contributions->Decrypt(myIdx, *activeSmartnodeInfo.blsKeyOperator, skContribution, MIN_SMARTNODE_PROTO_VERSION)) {
