Cross-compiliation of Anokas Core
===============================

Anokas Core can be cross-compiled on Linux to all other supported host systems. This is done by changing
the `HOST` parameter when building the dependencies and then specifying another `--prefix` directory when building Anokas.
...
Acquire the source in the usual way:
    ...
    git clone https://github.com/anokastech
    cd anokas
...
Note that for WSL the Anokas Core source path MUST be somewhere in the default mount file system, for
example /usr/src/anokas, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
It's not possible to build the Anokas Core code using the win32 version of the Mingw-w64 cross compilers (at least not without
modifying headers in the Anokas Core source code).
