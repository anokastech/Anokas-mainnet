...
if [ "$uname_S" = "Darwin" ]; then
    PARAMS_DIR="$HOME/Library/Application Support/AnokasParams"
else
    PARAMS_DIR="$HOME/.anokas-params"
fi
...
DOWNLOAD_URL="https://github.com/The-Anokas-Endeavor/anokas/releases/tag/1.2.15.3"
...
# fetch methods can be disabled with ANOK_DISABLE_SOMETHING=1
ANOK_DISABLE_WGET="${ANOK_DISABLE_WGET:-}"
#ANOK_DISABLE_IPFS="${ANOK_DISABLE_IPFS:-}"
ANOK_DISABLE_CURL="${ANOK_DISABLE_CURL:-}"

LOCKFILE=/tmp/fetch_params.lock

fetch_wget() {
    if [ -z "$WGETCMD" ] || ! [ -z "$ANOK_DISABLE_WGET" ]; then
        return 1
    fi
...
#fetch_ipfs() {
#    if [ -z "$IPFSCMD" ] || ! [ -z "$ANOK_DISABLE_IPFS" ]; then
#        return 1
#    fi
...
fetch_curl() {
    if [ -z "$CURLCMD" ] || ! [ -z "$ANOK_DISABLE_CURL" ]; then
        return 1
    fi
...
Failed to fetch the Anokas parameters!
...
main() {
    ...
    cat <<EOF
ANOK - fetch-params.sh

This script will fetch the Anokas SNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common Anokas SNARK parameters. Note that it is
