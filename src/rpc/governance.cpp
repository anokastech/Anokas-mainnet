...
UniValue gobject_prepare(const JSONRPCRequest& request)
{
    ...
    // This command is dangerous because it consumes 5 ANOKAS irreversibly.
    // If params are lost, it's very hard to bruteforce them and yet
    // users ignore all instructions on anokascentral etc. and do not save them...
...
void gobject_vote_conf_help()
{
    throw std::runtime_error(
                ...
                "Vote on a governance object by smartnode configured in anokas.conf\n"
                "\nArguments:\n"
...
UniValue gobject_vote_conf(const JSONRPCRequest& request)
{
    ...
    if (!dmn) {
        ...
        resultsObj.push_back(Pair("anokas.conf", statusObj));
...
#endif // ENABLE_WALLET
            "  vote-conf          - Vote on a governance object by smartnode configured in anokas.conf\n"
#ifdef ENABLE_WALLET
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ ----------
    /* Anokas features */
    { "anokas",               "getgovernanceinfo",      &getgovernanceinfo,      true,  {} },
    { "anokas",               "getsuperblockbudget",    &getsuperblockbudget,    true,  {"index"} },
    { "anokas",               "gobject",                &gobject,                true,  {} },
    { "anokas",               "voteraw",                &voteraw,                true,  {} },
