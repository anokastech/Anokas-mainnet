...
void CPrivateSendServer::ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, CConnman& connman)
{
    ...
    if (fLiteMode) return; // ignore all Anokas related functionality
...
/*
    ...
    allow endless transaction that would bloat Anokas and make it unusable. To
...
bool CPrivateSendServer::IsInputScriptSigValid(const CTxIn& txin)
{
    ...
    if (nTxInIndex >= 0) { //might have to do this one input at a time?
        ...
        // TODO we're using amount=0 here but we should use the correct amount. This works because Anokas ignores the amount while signing/verifying (only used in Bitcoin/Segwit)
        if (!VerifyScript(txNew.vin[nTxInIndex].scriptSig, sigPubKey, &txNew.vin[nTxInIndex].scriptWitness, SCRIPT_VERIFY_P2SH | SCRIPT_VERIFY_STRICTENC, MutableTransactionSignatureChecker(&txNew, nTxInIndex, 0))) {
...
{
    if (fLiteMode) return;        // disable all Anokas specific functionality
