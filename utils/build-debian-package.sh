...
PACKAGE_VERSION=$($SRC_PATH/src/anokasd --version | grep version | cut -d' ' -f4 | tr -d v)
...
# Copy binaries
cp $SRC_PATH/src/anokasd $DEB_BIN
cp $SRC_PATH/src/anokas-cli $DEB_BIN
cp $SRC_PATH/util/fetch-params.sh $DEB_BIN/anokas-fetch-params
# Copy docs
...
# Copy manpages
cp $SRC_DOC/man/anokasd.1 $DEB_MAN
cp $SRC_DOC/man/anokas-cli.1 $DEB_MAN
cp $SRC_DOC/man/anokas-fetch-params.1 $DEB_MAN
# Copy bash completion files
cp $SRC_PATH/contrib/anokasd.bash-completion $DEB_CMP/anokasd
cp $SRC_PATH/contrib/anokas-cli.bash-completion $DEB_CMP/anokas-cli
# Gzip files
gzip --best -n $DEB_DOC/changelog
gzip --best -n $DEB_DOC/changelog.Debian
gzip --best -n $DEB_MAN/anokasd.1
gzip --best -n $DEB_MAN/anokas-cli.1
gzip --best -n $DEB_MAN/anokas-fetch-params.1

cd $SRC_PATH/contrib

# Create the control file
dpkg-shlibdeps $DEB_BIN/anokasd $DEB_BIN/anokas-cli
